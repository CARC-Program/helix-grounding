...
flags a net you named that only connects to one thing — a label that was
typed and never joined anything, which looks identical to a real one on a
printout.
...
def _kicad_community(facts: ProjectFacts) -> str:
    ...
    return f"""
**A BOM/netlist checker that handles KiCad's export quirks — looking for files that break it**
...
Nets Eeschema named itself — `unconnected-(U1-Pad7)`, `Net-(R3-Pad1)` — are
left alone, since those are normal and you already get told about them.
...
def _eevblog(facts: ProjectFacts) -> str:
    return f"""
**Offline BOM checker — asking this forum to find its failure modes**
